...
void *mm_malloc(size_t size) {
    ...
    if (block_size(curr) >= size) { // if the size is large enough to malloc
        block_t *alloc = curr;
        pull_free_block(curr); // pulls free block
        if (size > MINBLOCKSIZE && block_size(curr) - size > MINBLOCKSIZE) { // condition to check for splitting
    ...
    if (curr == flist_first) { // since the list is circular -- if the head is reached again
        break;
    }
    }
    new = mem_sbrk(size); // otherwise, if there is no memory, asks for more (can't find a fit)
    ...
    block_set_size_and_allocated(epilogue, TAGS_SIZE, 1); // sets epilogue to be allocated
...
/**
 * Helper function for mm_free(), coalesces free blocks surrounding a newly freed block
 *
 * Parameters:
...
block_t* coalesce(void *b) {
    ...
    if (!(block_prev_allocated(t)) && !(block_next_allocated(t))) { // if next and prev are unallocated (free)
       // then, add up both blocks, and puts in new block of that size into the free list
        // gets sizes of all three blocks, including the current one
       ...
       // pulls blocks and sets them to be unallocated
        ...
        block_set_size(prev, (one+two+three)); // changes size of prev block (so it is all one large free block)
       // set pointer to prev
        t = prev;
    }
...
void *mm_realloc(void *ptr, size_t size) {
    ...
    if (((original - requested) >= MINBLOCKSIZE) && (requested <= (original / 2))) { // splits if requested size smaller than ptr's size
        block_set_size(block, requested);
        block_t *freed = block_next(block);
        block_set_size_and_allocated(freed, original-requested, 0); // splitting- taking (original size - requested size)
        insert_free_block(freed); // inserts this new block into free list
        return ptr;
    }
    size_t to_check = original + block_size(block_next(block)); // checks if the size of the next block is large enough
    if (!block_next_allocated(block) && (requested <= to_check)) { // if next block is unallocated and original + next big enough
        ...
        if (requested < original) { // if original size large enough
            void *to_return = memcpy(ret, to_save, requested); // copies over memory, passing in requested size
            return to_return;
        }
